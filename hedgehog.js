...
Hedgehog.prototype = {
  ...
  compile: function(data, path) {
    ...
    var namespace = self.conf.namespace + " = " + self.conf.namespace + " || {}; ";

    var compiled_template = 'templates.' + template_ns + " = new Hogan.Template("
      + hogan.compile(data.toString(), {asString: true})
      + ');';

    var output = namespace + compiled_template;


    if (self.conf.amd){
      output = namespace + 'define(["' + self.conf.hogan_path + '"], function(Hogan){' + compiled_template + ' return templates.'+template_ns +'});';
    }

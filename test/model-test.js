...
describe('model', function () {
	...
	describe('Friend stuff', function(){
		...
		beforeEach( function (done){
			async.map([{name: 'User2'},{name: 'User4'},{name: 'User1'}],User.findOne, function (err, users){
				var user	= users[0],
					friend	= users[1],
					friend2 = users[2];
		...
		it('should add a friend request', function (done) {
			async.map([{name: 'User2'},{name: 'User4'}],User.findOne, function (err, users){
				var user	= users[0],
					friend	= users[1];
		...
		function acceptFriend(cb){
			User.findOne({name: 'User4'}, function (err, doc){
				User.acceptFriend(doc, doc.friends[0], function(err, friendreq){
					async.map([{name: 'User2'},{name: 'User4'}],User.findOne, cb);
				});
			});
		}
